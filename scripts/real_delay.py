...
def parse_uri(uri:str)->dict:
    ...
    if s=="vless":
        ...
        return {"scheme":"vless","server":p.hostname,"port":p.port or 443,"uuid":urllib.parse.unquote(p.username or ""),"network":first(q,"type",default="tcp").lower(),"path":first(q,"path",default="/"),"host":first(q,"host"),"security":sec,"sni":first(q,"sni","serverName",default=p.hostname),"fp":first(q,"fp",default="chrome"),"pbk":first(q,"pbk"),"sid":first(q,"sid"),"service_name":first(q,"serviceName"),"authority":first(q,"authority")}
    if s=="vmess":
    ...
    if s=="trojan":
        return {"scheme":"trojan","server":p.hostname,"port":p.port or 443,"password":urllib.parse.unquote(p.username or ""),"network":first(q,"type",default="tcp").lower(),"path":first(q,"path",default="/"),"host":first(q,"host"),"sni":first(q,"sni",default=p.hostname),"service_name":first(q,"serviceName")}
    if s=="ss":
...
def xray_outbound(n:dict,tag:str)->dict:
    ...
    if s=="vless":
        ...
        if n.get("security")=="tls":
            ...
            stream["security"]="reality"; stream["realitySettings"]={"serverName":n.get("sni") or n["server"],"fingerprint":n.get("fp") or "chrome","publicKey":n.get("pbk") or "","shortId":n.get("sid") or "","spiderX":"/"}
...
def socks_http(port,host,path,use_tls,status,body,timeout):
    ...
    try:
        ...
        conn.sendall(f"GET {path} HTTP/1.1\r\nHost: {host}\r\nConnection: close\r\nUser-Agent: AhmedVPN-RealDelay/5\r\n\r\n".encode())
        data=bytearray()
...
def main():
    ...
    start=time.perf_counter(); included=[]; failures=[]
    with tempfile.TemporaryDirectory(prefix="real-delay-") as td:
        ...
        if check.returncode!=0: raise SystemExit("Xray multi-outbound config rejected: "+(check.stderr or check.stdout)[-4000:])
    ...
    by_index={r["index"]:r for r in results}; final=[]
    for idx,item in enumerate(pool):
        r=by_index.get(idx)
        if r is None: final.append({**item,"msft_ok":False,"google_204_ok":False,"internet_healthy":False,"delay_ms":-1,"details":{"config_conversion_failed":next((x["reason"] for x in failures if x["index"]==idx),"not-tested")}})
    ...
    meta=OUT/"metadata"; meta.mkdir(parents=True,exist_ok=True); alive=sum(1 for r in final if r["msft_ok"] or r["google_204_ok"]); healthy=sum(1 for r in final if r["internet_healthy"])
    (meta/"real_delay.json").write_text(json.dumps({"schema":4,"generated_at":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"engine":"Xray","mode":"single_long_lived_process_multi_outbound","candidates":len(pool),"compatible":len(included),"config_conversion_failed":len(failures),"alive":alive,"internet_healthy":healthy,"dead":len(final)-alive,"workers":workers,"timeout_s":timeout,"elapsed_s":round(time.perf_counter()-start,2),"results":final},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    print(json.dumps({"selected":len(pool),"compatible":len(included),"config_conversion_failed":len(failures),"alive":alive,"internet_healthy":healthy,"dead":len(final)-alive,"workers":workers,"elapsed_s":round(time.perf_counter()-start,2)},ensure_ascii=False))
